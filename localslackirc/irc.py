...
class Server:
    ...
    @registered_command
    @parse_args('message', minargs=0)
    async def cmd_away(self, message: Optional[str] = None) -> None:
        if message:
    ...
    async def cmd_topic(self, channel_name: str, topic: Optional[str] = None) -> None:
    ...
    async def cmd_kick(self, channel_name: str, nickname: str, message: Optional[str] = None) -> None:
        try:
            channel = await self.sl_client.get_channel_by_name(channel_name[1:])
        except KeyError:
            return await self.sendreply(Replies.ERR_NOSUCHCHANNEL, channel_name, 'No such channel')
